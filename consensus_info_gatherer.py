...
def days_since_start(date): 
    target_date = datetime.date(2022, 10, 19)
    difference = date - target_date
    return difference.days

picks_dict = {'date': [], 'dict_string': []}
with open("consensus stuff.txt") as file: 
    f = file.readlines()
...
for line in f: 
    line = line.rstrip('\n')
    if len(line.split('-')) == 3 and line.split('-')[0].isnumeric():
        ...
        cur_date = datetime.date(int(line.split('-')[0]),int(line.split('-')[1]),int(line.split('-')[2]))
    else: 
