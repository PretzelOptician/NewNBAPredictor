...
def convert_team_name(team, year): 
    ...
    elif team == 'New Jersey': return 'New Jersey Nets'
    ...
    elif team == 'OklahomaCity' or team == 'Oklahoma City' or team == 'Seattle Supersonics': 
        if year < 2009: return 'Seattle Supersonics'
...
print("NOW GENERATING SPREAD DATA")

for yearOffset in range(16):
    year = 2008+yearOffset

    if os.path.exists(f'../historic_data/{year}_spread_data.xlsx'): 
        print("Found existing data for year " + str(year) + ", loading data now...")
        new_data = pd.read_excel(f'../historic_data/{year}_spread_data.xlsx')
            ...
            if row%2==1: 
                ...
                if not (((games_played_team_one > 81 or games_played_team_two > 81)) or (year == 2020 and (games_played_team_one > 63 or games_played_team_two > 63)) or (year == 2021 and (games_played_team_one > 71 or games_played_team_two > 71)) or (year==2012 and (games_played_team_one > 65 or games_played_team_two > 65))): 
                    ...
                    if not push and not outlier: 
                        new_data.loc[len(new_data.index)] = [year, (1 if home_spread_hit else 0), total, spread, team1, team2, None, None, ppg1, ppg2, pace1, pace2, ortg1, ortg2, drtg1, drtg2, drb1, drb2, threePAR1, threePAR2, ts1, ts2, ftr1, ftr2, d_tov1, d_tov2, o_tov1, o_tov2, ftperfga1, ftperfga2, points_over_average_ratio1, points_over_average_ratio2, hotness_ratio1, hotness_ratio2, std_dev1, std_dev2, win_pct1, win_pct2, rsw1, rsw2, rating1, rating2, win_pct_close1, win_pct_close2, None, None, mov_a_h, mov_a_a, injury_gmsc1, injury_gmsc2, injury_mins1, injury_mins2]
        ...
        new_data.to_excel(f'../historic_data/{year}_spread_data.xlsx')
